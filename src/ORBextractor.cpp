...
void ORBextractor::ComputePyramid(cv::Mat image) {
  ...
  for(int level = 0;level<nlevels;++level){
    float scale = mvScaleFactor[level]; // get scale form mvScaleFactor
    // compute target scale size of image
    Size sz(cvRound((float)image.cols*scale), cvRound((float)image.rows*scale));
    Size wholeSize(sz.width+EDGE_THRESHOLD*2,sz.height+EDGE_THRESHOLD*2); // enlarge the image range for calculate descriptor
    Mat temp(wholeSize,image.type()),masktemp;
    mvImagePyramid[level] = temp(Rect(EDGE_THRESHOLD,EDGE_THRESHOLD,sz.width,sz.height)); // get real image range

    // compute the resized image
    if ( level != 0){
      resize(mvImagePyramid[level-1],mvImagePyramid[level],sz,0,0,INTER_LINEAR); // use pre-level to init this level

      // expend border
      copyMakeBorder(mvImagePyramid[level],temp,EDGE_THRESHOLD,EDGE_THRESHOLD,EDGE_THRESHOLD,EDGE_THRESHOLD,BORDER_REFLECT_101+cv::BORDER_ISOLATED);

    }
    else{
      // first image to init whole pyramid
